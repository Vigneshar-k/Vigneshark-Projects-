	...
	private int initializeProducts()throws IOException
{
    ...
    try
    {
        ...
        // First, let's check if there are products in the database
        PreparedStatement checkPs = con.prepareStatement("SELECT COUNT(*) FROM products");
        ResultSet checkRs = checkPs.executeQuery();
        if(checkRs.next()) {
            System.out.println("Number of products in database: " + checkRs.getInt(1));
        }
        ...
        // Clear existing lists to prevent duplicate entries
        pid.clear();
        name.clear();
        type.clear();
        qty.clear();
        ...
        if(rs.last()) {
            x=rs.getRow();
            System.out.println("Number of rows found: " + x);
            rs.beforeFirst();
        }
        ...
        while(rs.next())
        {
            ...
            System.out.println("Added product: " + rs.getString(2)); // Debug line
        }

        System.out.println("Final size of pid list: " + pid.size());
        return x;
    }
    catch(Exception e)
    {
        System.out.println("Error in initializeProducts: ");
        e.printStackTrace();
        return 0;
    }
}
